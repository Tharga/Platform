...
- **Scope reference** - `ScopeView` renders a read-only, sortable grid of the configured scopes — each scope's **description**, the **access levels** that grant it, and the **tenant roles** that grant it. Built dynamically from `IScopeRegistry` / `ITenantRoleRegistry`, so it always reflects the live configuration (no hard-coded list). Parameters: `ShowDescription` (default on), `ShowRoles` (default on; the column auto-hides when no tenant roles are configured), `AllowGridSorting` (sort by scope name, default on), `AllowGridFiltering` (case-insensitive name filter, default off). Shows a friendly notice when no scopes are configured.
