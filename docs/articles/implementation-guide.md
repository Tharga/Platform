...
| `<UsersView />` | Admin user list: last seen, directory verification, user deletion, and a directory-only tab when a directory service is registered. Viewing and acting require the `users:manage` system scope — enforced in the service layer. Its **Teams** tab additionally offers deleting any team to a holder of the `teams:delete` system scope, which no consent option grants (see [User management & directory](user-management.md)) |
