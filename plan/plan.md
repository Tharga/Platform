...
- [x] 2. Baseline build + test — clean build, 974 tests pass.

- [x] 3. Failing tests for defect 1 — both failed before the fix, as required.
...
- [x] 4. Guard `SetMemberRoleAsync` in `TeamServiceBase`
...
- [x] 5. Provenance approach — **(a) chosen**: distinct claim type.
...
- [x] 6. Implemented `TeamClaimTypes.SystemScope`
...
- [x] 7. Tests for defect 2 — new `ScopeProvenanceTests` (7).
...
- [~] 9. Full suite green at 985. **Docs review still to do.**
...
2026-07-27 — **Both defects fixed and committed** (`8a4a4b8`). Suite green at 985 (baseline 974, +11).

What was done:
- Guards in `TeamServiceBase.SetMemberRoleAsync` refusing both an Owner grant and an Owner demotion. In the
  domain, not the decorator — it is an invariant. `TransferOwnershipAsync` unaffected because it calls the
  protected `SetTeamMemberRoleAsync`.
- `TeamClaimTypes.SystemScope` added. Emission split at `TeamServerClaimsTransformation` (app roles) and
  `ApiKeyAuthenticationHandler`'s system-key branch; team grants keep `Scope`. Consumption follows through
  `TeamScopePolicy` / `TeamScopeGate`, plus `TeamVisibility`, the three `users:manage` views,
  `McpScopeChecker`, and `AuditLogView` (unpinned now requires a system grant — that is the cross-tenant fix).
- 21 existing tests failed on the split, all positive fixtures granting a system scope under the old claim
  type. No negative test started passing. Mixed fixtures gained an explicit system channel rather than a
  blanket swap.
- Sample: `audit:read` returned to the Developer map, now meaning a system-wide grant specifically.
  `apikey:manage` stays out.

**Next: step 9 docs review**, then step 10 close-out. `docs/articles/user-management.md` and
`implementation-guide.md` describe the scope model; the team-vs-system distinction is now load-bearing.

**Breaking change for the release notes:** consumers reading `TeamClaimTypes.Scope` directly to find a system
scope must read `TeamClaimTypes.SystemScope` instead.
