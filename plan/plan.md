...
- [x] **6b. Test-feedback fixes + user icons (scope pulled from Phase B per user, 2026-07-24)**
  - **Selector broken-image fix:** `TeamSelector` rendered `RadzenImage Path="@SelectedTeam.Icon"` — `Icon` is now a store *reference*, not a URL → broken link. Routed through `<TeamAvatar>` (resolver → endpoint URL, initials fallback).
  - **Title-bar avatar:** moved `<TeamAvatar>` into the `ExpandableCard` `TopMenu` (title bar) instead of the body action row. (`ExpandableCard` title only supports a font Icon + string Text; `TopMenu` is the render-fragment slot in the title bar. Left-of-name would need a Tharga.Blazor change — deferred.)
  - **User icons (profile upload as a Gravatar alternative):** `IUser.Icon` (opt-in default member); `IUserService.SetOwnIconAsync`/`ClearOwnIconAsync` (self-service — operate on the current user, decorator passes through) orchestrated in `UserServiceBase` via the threaded `IIconStore` (+ `SetUserIconReferenceAsync` → `UserRepository.SetIconAsync`, opt-in by entity shape). Built-in `GravatarIconSource` registered LAST (users with email → Gravatar; teams/no-email → null), so an uploaded icon overrides Gravatar which overrides initials. `<UserAvatar>` (round) + `UserIconDialog` (upload/URL/"Use Gravatar"); `UserProfileView` now shows the avatar + "Change picture" (hidden if no store). Sample `UserEntity` gains `Icon`; sample `UserService` ctor takes `IIconStore`. 18 new tests (Gravatar source, self-service orchestration). Full suite 825 green.
